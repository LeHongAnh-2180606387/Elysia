    ...
    public class FCGWaypointsContainer : MonoBehaviour
    {
        ...
        public void RefreshAllWayPoints()
        {
            ...
            if (!trafficSystem)
            {
...
#if UNITY_EDITOR
                // Chỉ chạy trong Editor để load TrafficSystem từ AssetDatabase
                if (!trafficSystem)
                ...
                // Trong runtime, log lỗi nếu không tìm thấy TrafficSystem
                if (!trafficSystem)
                {
                    Debug.LogError("Traffic System not found. Make sure it exists in the scene or load it at runtime.");
                    return;
                }
            }
        ...
        void OnDrawGizmos()
        {
            ...
            if (!isPlay && wCount > 1)
            {
                ...
                if ((waypoints[wCount - 1].localPosition != nodeEnd) || (waypoints[0].localPosition != nodeBegin))
                {
                    ...
                    // Logic chạy trong runtime (khi game được build)
                    if (transform.hasChanged)  // Kiểm tra xem có sự thay đổi vị trí của đối tượng không
                    {
                        RefreshAllWayPoints();
                    }
                }
